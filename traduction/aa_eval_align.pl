...
while($line = <SUBMISSIONFILE>) {
    ...
    ### skipping sentences that are not in reference. 
    if ( ! exists $sentences{$components[0]}){
	#print STDERR "skipping $components[0]\n";
	next;
    }
    # identify the S[ure] alignments
    if( scalar (@components) == 3 ||
...
$num_keys = scalar keys %sentences;
print STDOUT "num sentences in reference : $num_keys\n";
...
if(scalar(keys %sureSubmission) + scalar(keys %probableSubmission) != 0) {
    ...
    print "probableMatchSure : $probableMatchSure\n";
    print "probableMatch  : $probableMatch\n";
    print "sureMatch  : $sureMatch\n";
    print  "#sureSubmission : ".scalar(keys %sureSubmission) . "\n";
    print  "#probableSubmission : ".scalar(keys %probableSubmission) . "\n";
    print  "#sureAnswer : ".scalar(keys %sureAnswer) . "\n";
    #print "sure precision :  ". (($probableMatchSure + $probableMatch) / scalar(keys %sureSubmission)  ) ;
...
printf("-----------------------------------\n");
$alpha = 0.5;
$falpha = 1/( (($alpha)/$probablePrecision) + ((1-$alpha)/$sureRecall) );
printf("   F-%1.1f     = %5.4f\n",$alpha,$falpha);
$alpha = 0.2;
$falpha = 1/( (($alpha)/$probablePrecision) + ((1-$alpha)/$sureRecall) );
printf("   F-%1.1f     = %5.4f\n",$alpha,$falpha);
